...
description: Use os comandos gh agent-session para inspecionar e gerenciar sessões de agente.
---


# Extensão de Sessão de Agente da CLI do GitHub

A extensão `gh agent-task` cria sessões de codificação do GitHub Copilot a partir da CLI. Uma sessão é uma issue do GitHub que aciona alterações de código automatizadas a partir de instruções em linguagem natural.

**Repositório**: https://github.com/github/agent-task (extensão interna do GitHub)

## Visão Geral

Sessões de agente são issues do GitHub que:
...
Crie uma nova sessão de agente a partir de uma descrição:
...
**Formato de Saída:**
O comando gera a URL da sessão de agente criada:
```
https://github.com/owner/repo/issues/123
```

### Listar Sessões de Agente

Liste sessões de agente em um repositório:

```bash
# Liste todas as sessões de agente
gh agent-task list
...
### Visualizar Sessão de Agente

Visualize detalhes de uma sessão de agente específica:
...
Atualize uma sessão de agente existente:
...
Descrições de sessão de agente devem ser instruções claras e específicas em linguagem natural:
...
A extensão `gh agent-task` é usada pelo recurso de saída segura `create-agent-session` no GitHub Agentic Workflows (gh-aw).
...
safe-outputs:
  create-agent-session:
    base: main                       # Branch base para o PR da sessão do agente
...
Quando uma issue for etiquetada com "code-task", analise os requisitos e crie uma sessão de agente de codificação GitHub Copilot com instruções detalhadas para implementar as alterações solicitadas.
```
...
A criação de sessão de agente requer permissões elevadas além do `GITHUB_TOKEN` padrão:

**Permissões Necessárias:**

- `contents: write` - Para criar branches e commits
- `issues: write` - Para criar a issue de sessão do agente
...
```
Erro: falha ao criar sessão de agente
...
Quando `safe-outputs.staged: true`, sessões de agente são pré-visualizadas sem criação:
...
```markdown
## 🎭 Modo Staged: Preview de Criação de Sessões de Agente

As seguintes sessões de agente seriam criadas se o modo staged estivesse desabilitado:
...
Quando a issue for etiquetada com "needs-implementation", crie uma sessão de agente com instruções de implementação.
```
...
Analise a base de código por oportunidades de melhoria e crie sessões de agente para as 3 principais melhorias.
```
...
Crie sessão de agente no repositório backend para implementar as alterações de API descritas nesta issue.
```
...
5. **Defina Restrições**: Observe quaisquer requisitos de compatibilidade ou limitações
...
1. **Monitore o Uso**: Rastreie taxas de criação e conclusão de sessão de agente
...
**Sintoma**: Sem erro, mas nenhuma tarefa criada
...
Quando usada via saídas seguras, o job create-agent-session fornece saídas:
  ...
  follow_up:
    ...
    steps:
      - name: Notificar equipe
        run: |
          echo "Sessão de agente criada: ${{ needs.create_agent_task.outputs.task_url }}"
```
